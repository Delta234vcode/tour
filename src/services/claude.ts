...
СПЕЦІАЛІЗАЦІЯ — не змішуй ролі; рівно 2 допоміжні агенти (соцмережі та факти з вебу для метрик — окремо через Gemini + Google Search, не через цей JSON):
• **perplexity** (+ optional perplexity_extra): ТІЛЬКИ веб, концертна хронологія, тури, дати ДД.ММ.РРРР, зали, setlist/songkick/bandsintown/ticket pages — рядок = URL події або н/д.
• **grok** (+ optional grok_extra): ТІЛЬКИ X/Twitter базз, сентимент фанів, «come to [city]», обговорення — кожен факт з URL. НЕ доручай тур-таблиці (це Perplexity) і НЕ доручай соцметрики (це Gemini).
...
Згенеруй JSON: perplexity, grok, perplexity_extra, grok_extra (останні два — "" якщо без другого проходу). Дотримуйся спеціалізації кожного агента. Базові промпти — по 3–8 речень; *_extra — лише якщо потрібен таргет-доп. Включи: без URL — н/д.`,
      },
    ],
  });
}
...
Згенеруй JSON: perplexity, grok, perplexity_extra, grok_extra (порожні рядки якщо не треба другого проходу). Скан САМЕ цих міст; спеціалізація: Perplexity = події/зали/веб, Grok = X/Twitter базз по містах, соцметрики = Gemini (не в цьому JSON). URL або н/д.`,
      },
    ],
  });
}
